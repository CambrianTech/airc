...
//!    pre-#885 binary against post-#885 schema. The comparison is
//!    DIRECTIONAL (git ancestry, not `!=`): a mismatch names which
//!    side is actually behind, because a current binary against a
//!    lagging checkout is a note about the checkout, not a stale
//!    binary. See [`classify_source_drift`].
//!
...
fn render_source_drift(state: SourceDriftState) -> Finding {
    match state {
        SourceDriftState::Match => Finding::ok(
            "binary",
            "installed binary matches source checkout HEAD",
        ),
        SourceDriftState::BinaryBehind { binary, source } => Finding::warn(
            "binary",
            format!("installed binary is BEHIND its source checkout (binary={binary} source={source})"),
        ...
        SourceDriftState::Diverged { binary, source } => Finding::warn(
            "binary",
            format!("installed binary and source checkout have diverged (binary={binary} source={source}) — neither reaches the other"),
        ...
        SourceDriftState::Unknowable { binary, source } => Finding::info(
            "binary",
            format!(
                "source checkout has no record of the binary's commit (binary={binary} source={source}); \
                 can't judge which side is stale"
            ),
        ),
    }
}
