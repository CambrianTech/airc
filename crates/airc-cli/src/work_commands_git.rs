//! Local `git` + repo-id operations — `spawn_claim_worktree` and the
//! helpers it composes. Currently shells out via `std::process::Command`;
//! card dec35ec7 will migrate these to the typed `GitClient` trait
//! from `airc-lib::tools` once that lands.
//!
//! Card c0bd865c phase 3 (c63a1b6e). The extracted helpers feed the
//! claim flow: when an agent claims a card, spawn_claim_worktree
//! resolves the repo root, derives a branch name from the card id +
//! slugified title, and runs `git worktree add` so two agents on the
//! same machine don't fight over the shared checkout. The git helpers
//! are also used by gh.rs (next phase) — `git_rev_parse_branch` and
//! `git_show_format` feed into `open_pr_and_link`.
