...
//!
//! Card c0bd865c sub-module — extracted from `work_commands.rs` to
//! shrink that file's 1880 LOC into something embedders can read in
//! one pass. The print_* functions are PURELY terminal formatting —
//! no substrate calls, no state mutation — so the split is clean.
//!
//! Subsequent extractions (lifecycle / git / gh / review) are
//! follow-up cards once the pattern is established.
