...
mod tests {
    ...
    // what this catches: an explicit `$AIRC_RUNTIME_DIR` must be honoured VERBATIM — the
    // override exists for test isolation, and a resolver that "helpfully" reinterprets it
    // fragments the machine-singular daemon it is meant to isolate.
    //
    // Asserted through `resolve_runtime_dir`, the pure function extracted for exactly this
    // purpose, rather than by mutating process env. The previous version called
    // `set_var` behind three `SAFETY: tests run serially` comments — cargo runs tests in
    // PARALLEL by default, so that reasoning was false and the mutation raced every other
    // test reading env. The pure seam was already sitting there unused.
    #[test]
    fn an_explicit_runtime_dir_override_is_honoured_verbatim() {
        let explicit = std::path::PathBuf::from("/tmp/airc-runtime-override");
        let resolved = resolve_runtime_dir(
        ...
        assert_eq!(
            resolved, explicit,
            "the override is taken as given, never reinterpreted"
        );
    }

    // what this catches: with no override, the path is the machine-stable
    // `<home>/.airc/runtime` and nothing per-session ($TMPDIR, $XDG_RUNTIME_DIR) can
    // influence it — the property the signature exists to prove.
    #[test]
    fn the_default_is_the_machine_stable_home_path() {
        let resolved = resolve_runtime_dir(None, Some(std::ffi::OsString::from("/home/someone")))
            .expect("a home resolves");
        assert!(
            resolved.ends_with("runtime") && resolved.to_string_lossy().contains(".airc"),
            "expected <home>/.airc/runtime, got {}",
            resolved.display()
        );
    }
}
