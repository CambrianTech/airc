...
| Manual peer-spec paste (uuid:pubkey over a gist) | **Pairing phrase admits a machine to the GRID — no account involved** (Joel: accounts are for sharing only, never for your own machines). The existing 4-word mnemonic (`airc join oregon-uncle-bravo-eleven`) becomes THE pairing surface (speakable, scannable as QR); it carries enough to find one grid member, who introduces the newcomer to the rest (peer records gossip inside the grid). Works fully offline/LAN — no gh, no cloud, no third party. | Mnemonic join: YES. Intra-grid gossip enrolment: NO | 625abe6d follow-up |
...
4. **mDNS discovery + intra-grid gossip enrolment** — deletes pairing ceremony on LAN;
   the mnemonic remains as the one-time admission rite and for crossing into other grids.
   No account anywhere in the own-grid path.
