...
2. Add a first consumer-facing work subscription surface so agents can
   ...
   - Manager-loop slice: expose a typed `WorkManagerStatus` that
     composes the room's work-board projection and active roster into
     deterministic recommendations (`ClaimWork`, `RecoverStaleClaim`,
     `PublishAvailability`, `SeedBacklog`, `Wait`). The room is the
     scheduling boundary: a card only exists for agents subscribed to
     the room that received its `CardCreated` event. In Continuum terms,
     recipes/templates generate content contexts — chats, games, dev
     sessions, documents, projects — and those contexts map to rooms.
     Work generated by that content is emitted into that room's stream,
     so scope selects work automatically instead of relying on a global
     queue sorter. GitHub issues or roadmap entries are candidates, not
     truth, until an adapter reconciles them into room-scoped work
     events with evidence.
   - Follow-up: add the GitHub/roadmap sync adapter that creates or
     updates room-scoped work cards only after checking linked issue and
     PR evidence. This avoids the failure observed on 2026-05-25 where
     an open GitHub issue represented work already completed by merged
     Continuum PRs; blindly mirroring labels produced a stale card that
     an agent had to sniff-test and close.
