...
## Identity, Attestation & Decentralized Enrollment (Alpha Gap / Redesign Roadmap, 2026-05-27)

Captured from the design pass on credentials. The world (and the grid) is **zero trust**: never trust, always verify, assume breach. These are the layers that make a *for-hire fulfillment grid* possible — trust must be hardware + math, not policy. Relates to §8 (trust store), §12 (Forge-alloy), §13 (consumer embedding), Security Gaps. Separation of concerns: the **grid** owns trust/economics/policing; **Continuum** is freed to be persona cognition + evolution.

**The arc:** hardware-attested identity (Sybil-resistant *who*) → domain-separated signed assertions (non-repudiable *acts*) → Forge-alloy Merkle-chained contracts (tamper-evident *history*) → grid (where contracted work runs) → token settlement on top. Tamper-proof because each link is cryptographically bound to the one beneath, rooted in hardware. "Only way alt coins can work here": value needs real identities + tamper-evident chains or the ledger is forgeable.

**SHIPPED (this branch):** `airc-protocol::assertion` — domain-separated `IdentityAssertion` + `PeerKeypair::sign_assertion(signer,key_id,context,challenge)` + `verify`/`verify_with_pubkey`, re-exported as `Airc::sign_assertion`. Domain tag `airc-identity-assertion:v1`, length-delimited bytes, proven disjoint from frame canonical CBOR (WebAuthn `clientDataJSON.type` analogue). Raw key never exposed → hardware signer is a drop-in. Tested + clippy-clean.

**Gaps / roadmap, ordered (each builds on the prior; do reliability + clean arch FIRST):**

1. **Algorithm agility (COSE) — the enclave prerequisite.** Apple Secure Enclave does **P-256/ES256 only, never Ed25519** (SE generates non-exportable P-256 keypairs). airc is Ed25519 everywhere. Identity/signature/registry must go COSE-alg-agile: alg-tagged keys (ES256 `-7` + EdDSA `-8`), variable-size pubkeys (P-256 = 33-byte compressed SEC1, not 32 — current `verify_with_pubkey(&[u8;32])` is Ed25519-only), verify dispatch on alg. Ripples through registry + peer-spec format. **Testable without hardware** via the `p256` crate as a software SE stand-in on the sign side (verify path identical). Start from a committed base.
2. **Hardware-backed signer + attestation.** Attestation = the WebAuthn *registration/create* counterpart: at key birth an SE/TPM/StrongBox emits an attestation statement, verified at **`PeerKeyRegistry` enrolment** (the RP check), chained to manufacturer roots (a global trust anchor we get for free, do not run). **Mandatory for any contract-signing identity** (non-attested keys may chat, may not sign contract steps). "Hardware is god."
3. **Zero-trust local ingress.** Today a local client opens the daemon socket and *claims* `from_peer`; the daemon stamps it (implicit same-user-locality trust). Fine for software keys (attacker with same-user RCE already reads the key). Becomes **load-bearing the moment keys are hardware-bound** (SE key unreadable → claimed `from_peer` spoofable by any local process): the daemon must require proof-of-possession at attach (challenge → `sign_assertion("airc.daemon.attach", nonce)` → verify → bind connection to the verified peer). This is **verify-once-at-ingress applied to the local IPC seam** (mirror of cross-machine slice 4). Ships *with* attestation. Also: assertions must **expire**; sessions re-verify (not trust-once); capabilities enforce least-privilege at use.
4. **Automatic, decentralized, low-friction enrollment** (the hard onboarding battle: remote + automatic + no central server + novices never edit files). **Attestation is the unlock** — a device self-enrolls by *proving it is real hardware*; no human vouching, no central airc registry. UX = OAuth **device flow** (what GitHub/Claude Code/Codex do: `airc join` pops a browser login, SE keygen in the background, binding auto-published). **GitHub = bounded rendezvous, not the data plane**: immutable signed enrollment blobs on its CDN, sharded — bootstraps first peers, then **P2P gossip mesh** carries steady state (a billion users hear enroll/revoke *from peers*, not from GitHub; tracker → DHT → swarm). Bones exist: invite/join, `add_peer`, `trust_rotation`, the gh rendezvous used for Forge-alloy.
5. **Trust/revocation propagation at scale.** Signed enroll/revoke deltas gossiped P2P over the `trust_rotation` seam; Merkle/accumulator proofs so "prove I'm not revoked" stays cheap. (Genuinely hard; known-pattern, not a research void.)
6. **Ecosystem governance (App Store model).** attestation≈notarization, capabilities≈entitlements, reputation≈reviews, revocation≈pulling the app. **Reputation** = verifiable aggregate over attested contract *outcomes* (signed fulfillment receipts + dispute records, not self-claims); hard part = anti-gaming (collusion/wash-trading). **Unwinding bad actors** = compartmentalize (capability scoping bounds blast radius) + penalize (stake/slash) + revoke + adjudicate (the chain *is* the evidence) + **escrow/dispute windows before settlement** — crypto can prevent/bound/penalize but **cannot reverse already-settled value**, so hold value until on-chain fulfillment proof.

**Friction-correct identity by class:** humans = passkey/SE biometric (lowest friction + hardware-bound); agents/personas = **delegated + host-attested** (a human authorizes the persona once via a signed bounded-capability delegation; persona signs its acts with a host-SE key, no per-action biometric — attributable, bounded, revocable).

**Crypto posture (verified):** zero OpenSSL in the tree — `ed25519-dalek` + `rustls`/`ring` + `sha2`. Keep it that way (better, less-vulnerable libraries; no OpenSSL CVE surface).

**Test strategy — short, elegant, comprehensive (Joel, 2026-05-27):** one property per test, the fixture absorbs the ceremony. The in-process daemon fixture (`airc-lib/tests/common/mod.rs` `Machine`: real `DaemonState` + `run` on a socket, attach real SDK handles) makes a real daemon-restart failure scenario a ~40-line test (`daemon_reconnect.rs`). **Single-machine** correctness + reliability is comprehensively coverable today this way (convergence, off-ORM streaming, durable replay, hammer, RPC, header-filter, restart + live-stream reconnect/resume — all green). **Federation / grid** behaviors need a **grid-simulation harness: N in-process daemons (each = a node) wired over a cross-machine transport in one process** — `GridSim { nodes: Vec<Machine> }` + LAN/relay wiring, driven deterministically to test join/leave, cross-grid delivery, revocation propagation, reconnect-under-churn, and enrollment-at-scale as short tests. Lands with slice 4 (cross-machine), since it needs verify-once-at-ingress + the cross-machine transports.
