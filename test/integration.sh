...
scenario_scope() {
  ...
  local scope_override override_real
  override_real=$(cd /tmp && pwd -P)/airc-it-override
  scope_override=$(cd "$a" && AIRC_HOME=/tmp/airc-it-override "$AIRC" debug-scope 2>&1)
  [ "$scope_override" = "$override_real" ] && pass "AIRC_HOME overrides cwd detection" \
                                           || fail "override ignored: got '$scope_override'"

  # Derived name: platform prefix + basename + 4-char hash;
  # same-basename-different-dirs don't collide.
  local same_base_1="/tmp/airc-it-proj-alpha/src"
  local same_base_2="/tmp/airc-it-proj-beta/src"
  mkdir -p "$same_base_1" "$same_base_2"
  local name_1 name_2
  name_1=$(cd "$same_base_1" && AIRC_PLATFORM_KIND=ubuntu "$AIRC" debug-name 2>&1)
  name_2=$(cd "$same_base_2" && AIRC_PLATFORM_KIND=ubuntu "$AIRC" debug-name 2>&1)
  ...
  case "$name_1" in
    ubu-src-*) pass "autogenerated nick uses friendly 3-letter platform prefix" ;;
    *) fail "autogenerated nick missing platform prefix: '$name_1'" ;;
  esac
